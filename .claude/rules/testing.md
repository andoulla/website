...
- Cover every distinct component state with a `jest-axe` check (`axe` from `jest-axe`, `expect(await axe(container)).toHaveNoViolations()`), but fold the check into the existing test that already puts the component into that state rather than writing a separate axe-only test — e.g. add the assertion at the end of the test that already clicks into a given view, instead of a dedicated `'has no axe violations in that view'` test. Only add a standalone axe test when no functional test already produces that state.
